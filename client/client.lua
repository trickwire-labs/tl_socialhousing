...
local function updateContext()
    if debug then
        TriggerServerEvent(ResourceName..':debugPrint', string.format(Lang.updateContextDebug, cache.serverId, PlayerHouse))
...
local function updateOptions()
    if debug then
        TriggerServerEvent(ResourceName..':debugPrint', string.format(Lang.updateOptionsDebug, cache.serverId, PlayerHouse))
    end
    ...
    for houseKey, houseData in pairs(Config.Houses) do
        ...
        table.insert(houseSelectionOptions,
        {
            ...
            onSelect    = function ()
                ...
                if not success then ShowNotification(Lang.notificationTitle, Lang.houseChoosingError, 'error', 3000) return end
                ShowNotification(Lang.notificationTitle, string.format(Lang.houseChoosingSuccess, houseData.label), 'success', 3000)
    ...
    if debug then
        TriggerServerEvent(ResourceName..':debugPrint', string.format(Lang.houseUpdatingSuccessDebug, cache.serverId, PlayerHouse))
        TriggerServerEvent(ResourceName..':debugPrint', '------------------------------------------------------')
    end
...
lib.registerContext({
    ...
    options = {
        ...
        icon = 'key',
        onSelect = function()
            if lib.callback.await(ResourceName..':buyKey', false) then ShowNotification(Lang.notificationTitle, Lang.keyPurchaseSuccess, 'success', 4000) end
        end
        }
    }
})
