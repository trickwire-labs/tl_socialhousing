...
lib.callback.register(ResourceName .. ':buyKey', function()
    ...
    if accountMoney >= Config.HouseKeyPrice then
        if Inventory:CanCarryItem(source, Config.KeyItem, 1) then
            ...
            if not success then
                ShowNotification(source, Lang.notificationTitle, Lang.couldntBuyKey .. addItemResponses[response],
                    'error', 4000)
                return
            else
            ...
            ShowNotification(source, Lang.notificationTitle, Lang.couldntBuyKey .. Lang.inventoryWeightLimit, 'error',
                4000)
            return
        end
    else
...
AddEventHandler('onResourceStart', function(resourceName)
    ...
    if debug then
        ...
        lib.print.debug('--------------------------------------------')
        lib.print.debug(Lang.runningChecksDebug)
        lib.print.debug('--------------------------------------------')
    end
...
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
    SetPlayerRoutingBucket(playerId,
        GetResourceKvpInt(ResourceName .. ':' .. ESX.GetPlayerFromId(playerId).getIdentifier() .. ':routingBucket') or 0)
